...
def onnx_to_coreml(onnx_model, output):  # type: (IO[str], str) -> None
    ...
    hidden_state_prev_shape_lower = [1, 1, 64]
    hidden_state_prev_shape_upper = [1, 8, 64]

    cell_state_prev_shape_lower = [1, 1, 64]
    cell_state_prev_shape_upper = [1, 8, 64]
    ...
    coreml_model = convert(onnx_model_proto, minimum_ios_deployment_target='13', 
                           onnx_coreml_input_shape_map={'hidden_state_prev': hidden_state_prev_shape, 'cell_state_prev': cell_state_prev_shape, 'ldmks_2d': ldmks_2d_shape, 'ldmks_2d_norm_prev': ldmks_2d_norm_prev_shape},
                           onnx_coreml_output_shape_map={'is_signing': [1, 1], 'sign_sigma': [1, 1], 'hidden_state': [1, 1, 64], 'cell_state': [1, 1, 64], 'ldmks_2d_norm_next_prev': [1, 1, 95, 2], 'ldmks_v_max': [1, 1], 'sign_sigma': [1, 1]},
